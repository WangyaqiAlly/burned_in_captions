...
class SolverWrapper(object):
    def __init__(self, sess, network, imdb, roidb, output_dir, logdir, pretrained_dir,pretrained_model=None):
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        if restore:
            try:
                ckpt = tf.train.get_checkpoint_state(self.pretrained_dir)
        ...
        for iter in range(restore_iter, max_iters):
            ...
            # cv2.imwrite("data/results/test_triandata.jpg" , img)
            #
            # assert 0
            # draw_boxes( blobs['data'], image_name, blobs['gt_boxes'], scale)
        ...
        if last_snapshot_iter != max_iters-1:
            self.snapshot(sess, max_iters-1)
...
def train_net(network, imdb, roidb, output_dir, log_dir, pretrained_dir,pretrained_model=None, max_iters=40000, restore=False):
    ...
    with tf.Session(config=config) as sess:
        sw = SolverWrapper(sess, network, imdb, roidb, output_dir,pretrained_dir=pretrained_dir, logdir= log_dir, pretrained_model=pretrained_model)
        print('Solving...')
