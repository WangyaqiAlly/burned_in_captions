...
def readSpeechImages():
	labeledDataFolder = '../ICML2018/SpeechData/labeledData/'
	testDataFolder = '../ICML2018/SpeechData/testData/'
	genData = '../ICML2018/Record/GAN_Speech/generatedImages/'
	...
	for indLabel,label in enumerate(labelNames):
		currentLabelFolder = testDataFolder+label+'_labeled/'
		...
		currentLabelFolder = labeledDataFolder+label+'_labeled/'
		fileNames = os.listdir(currentLabelFolder)
	...
	currentFolder = genData
	fileNames = os.listdir(currentFolder)
	### The rest is unlabeled
	for i in range(len(fileNames)):
		img = cv2.imread(currentFolder+fileNames[i],cv2.IMREAD_GRAYSCALE).reshape(width, height,1)
