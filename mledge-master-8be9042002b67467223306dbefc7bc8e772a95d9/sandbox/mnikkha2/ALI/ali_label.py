...
########### read the labels
y_embeddings = np.zeros(shape = (202599,1,1, 40))
with open(args.labelsDir, 'r') as f:
	...
	for line in f:
		if i > 1:
			...
			y_embeddings[i-2,0,0,:] = line.split()[1:]
		i += 1
...
def gnet(args,z, embeddings, reuse=None):
	...
	with tf.variable_scope('gnet',reuse=reuse):
		...
		g = tf.layers.conv2d_transpose(inputs=z, filters=552, kernel_size=4, strides=[1,1], padding='valid', data_format='channels_last', activation=None) ; print g
...
embeddings = tf.placeholder('float32', [None,1,1,40], name = 'embeddings'); print embeddings
...
dgrads = dopt.compute_gradients(dloss,var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,'dxzynet'))
...
with tf.Session() as sess:
	...
	dn=0.
		...
		input_embeddings = y_embeddings[j:j+args.batch]
		input_z = np.random.randn(args.batch,args.m)
		input_z = input_z.reshape(args.batch,1,1,args.m)
	        _,ec_,en_ = sess.run([etrain,gloss,enorm], feed_dict={x:input_x,z:input_z,embeddings:input_embeddings})
	        _,gc_,gn_ = sess.run([gtrain,gloss,gnorm], feed_dict={x:input_x,z:input_z,embeddings:input_embeddings})
		_,dc_,dn_ = sess.run([dtrain,dloss,dnorm], feed_dict={x:input_x,z:input_z,embeddings:input_embeddings})
		ec=ec_
		gc=gc_
		dc=dc_
		en=en_
		...
		if j%1000 == 0:
		    xgen = sess.run(g_z, feed_dict={x:input_x, z:np.random.randn(args.batch,1,1,args.m), embeddings:input_embeddings})
