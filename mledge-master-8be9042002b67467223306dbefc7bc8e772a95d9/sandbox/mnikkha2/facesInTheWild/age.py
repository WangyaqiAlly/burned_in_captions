...
parser.add_argument('--m', help='latent space dimensionality', default=100, type=int)
...
parser.add_argument('--batch', help='batch size', default=250, type=int)
...
parser.add_argument('--dataFolder', help='data folder', default='./lfw2/')
...
for f in folderNames:
        fileNames = os.listdir(dataFolder+f)
        for f2 in fileNames:
                if '.jpg' not in f2:
                        continue
                img = cv2.imread(dataFolder+f+'/'+f2,0)
                d.append(img)


#numTrainSamples = len(d)
d = np.array(d[0:numFaces]).reshape(numFaces,250,250,1).astype('float32')
  ...
  for myDevice in gpuList:
    with tf.device(myDevice):
      with tf.variable_scope('gnet',reuse=reuse):
        ...
        g = tf.layers.conv2d(inputs=g, filters=args.n, kernel_size=3, strides=1,activation=tf.nn.elu, padding='same') ; print g
        g = tf.layers.conv2d(inputs=g, filters=args.n, kernel_size=3, strides=1,activation=tf.nn.elu, padding='same') ; print g
        g = tf.image.resize_bilinear(images=g,size=[250,250]) ; print g
        g = tf.layers.conv2d(inputs=g, filters=args.n, kernel_size=3, strides=1,activation=tf.nn.elu, padding='same') ; print g
        g = tf.layers.conv2d(inputs=g, filters=args.n, kernel_size=3, strides=1,activation=tf.nn.elu, padding='same') ; print g
...
for myDevice in gpuList:
    with tf.device(myDevice):
	x = tf.placeholder('float32', [None,250,250,1],name='x') ; print x
