...
train_dir = 'golden_100_0.01'
Scope_Prefix = 'golden_100_0.01_'
Total_iteration = 100000
Total_batchn = 390
Record_file = '../res/'+ Scope_Prefix + ('bn_%06d' % Total_batchn )+ ('_iter_%06d' % Total_iteration )+'record.txt'
acc_file_name = '../res/'+Scope_Prefix + ('bn_%06d' % Total_batchn )+ ('_iter_%06d' % Total_iteration )+'record_each_class.txt'
acc_file_name_train = '../res/'+Scope_Prefix + ('bn_%06d' % Total_batchn )+ ('_iter_%06d' % Total_iteration )+'record_each_class_train.txt'
mtx_file_name = '../res/'+Scope_Prefix + ('bn_%06d' % Total_batchn )+ ('_iter_%06d' % Total_iteration )+'confusion_mtx.txt'
...
class Train_site0(object):
    ...
    def generate_label(self):
        ...
        for f in fileNames:
            if 'img' not in f:
                continue 
            batch_data = np.load('../data_uncon_100/'+f)
            ...
            l = sess.run(predictions,feed_dict={self.test_image_placeholder: batch_data})
            #print l.shape
            l = np.argmax(l, axis = 1)
            #print l.shape
            targets = np.array([l]).reshape(-1)
            labels = np.eye(10)[targets]
            #print labels.shape
            np.save('../data_uncon_100/labels'+('%06d' % count), labels)
            count += 1
            if count % 100 == 0:
                print f
