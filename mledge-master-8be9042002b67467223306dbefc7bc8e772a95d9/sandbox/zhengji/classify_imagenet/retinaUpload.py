...
class Uploader(object):
    ...
    def calc_func(self, obj, dest_fun, train_images, train_labels):
        return obj.sess.run(dest_fun, feed_dict={
                    obj.x: train_images[:, 4:28, 4:28],
                    obj.y_: train_labels,
                    obj.keep_prob: 1.0
                    })
    ...
    def filt_and_transfer(self, filename, network_obj, rate=1.0, mode=0):
        ...
        s_num = [i for i in range(0,len(train_images), 1000)]
        e_num = [len(train_images) if s + 1000 > len(train_images) else s + 1000 for s in s_num]
        correct_prediction = np.zeros(len(train_images), dtype=np.int)
        #filt part
        for s, e in zip(s_num, e_num):
            correct_prediction[s:e] = self.calc_func(network_obj, network_obj.correct_prediction, train_images[s:e], train_labels[s:e])
        #correct_prediction = network_obj.sess.run(network_obj.correct_prediction, feed_dict={
        #    network_obj.x: train_images[:,4:28,4:28], 
        ...
        s_correct_num = [i for i in range(0,len(correct_images), 1000)]
        e_correct_num = [len(correct_images) if s + 1000 > len(correct_images) else s + 1000 for s in s_num]

        softmax = np.zeros((len(correct_images), 10))
        for s, e in zip(s_correct_num, e_correct_num):
            softmax[s:e] = self.calc_func(network_obj, network_obj.softmax, correct_images[s:e], correct_labels[s:e])

        cross_entropy = np.zeros((len(correct_images)))
        for s, e in zip(s_correct_num, e_correct_num):
            cross_entropy[s:e] = self.calc_func(network_obj, network_obj.cross_entropy_individual, correct_images[s:e], correct_labels[s:e])

        if mode==ENTROPY_MODE:
            #softmax = network_obj.sess.run(network_obj.softmax, feed_dict={
            #    network_obj.x: correct_images[:,4:28,4:28],
