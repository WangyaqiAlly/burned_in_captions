...
class RetinaNode(object):
    ...
    def train(self, total_epoch, record_file=None):
        ...
        while True:
            ...
            #self.visualize(batch[0], batch[1]) 
            images = (np.array(batch[0]) / 255.0) - 0.5
            ...
            if time.time() - time_stamp > 1:
                time_stamp = time.time()
                norm, train_loss = self.sess.run([self.norm, self.cross_entropy], feed_dict={
                    self.x:images, self.y_:batch[1], self.keep_prob:1.0})


            sys.stdout.write("\rstep %d, epoch %d, t_acc %8.4lf%% | queue: %d | loss: %.6lf | norm: %.4lf" %
                (i, epoch, test_accuracy*100, self._reader.train._resource_pool.qsize(), train_loss, norm))
            sys.stdout.flush()

            self.sess.run(self.train_step, feed_dict={
                self.x: images, self.y_:batch[1], self.keep_prob: 0.8})
    ...
    def test(self):
        ...
        i = 0
        accuracy = 0
        while i * 1000 < self._reader.test.images.shape[0]:
            s = i * 1000
            e = (i+1) * 1000
            if e > self._reader.test.images.shape[0]:
                e = self._reader.test.images.shape[0]
            batch = [self._reader.test.images[s:e], self._reader.test.labels[s:e]]
            images = (np.array(batch[0]) / 255.0) - 0.5
            accuracy += (e - s) * self.sess.run(self.accuracy, feed_dict={
                        self.x:images[:,4:28,4:28], 
                        self.y_:batch[1], 
                        self.keep_prob: 1.0})
            index += e - s
            i += 1
        return accuracy * 1.0 / index 
