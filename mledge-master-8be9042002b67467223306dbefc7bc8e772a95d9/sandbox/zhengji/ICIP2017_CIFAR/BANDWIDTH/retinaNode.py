...
class RetinaNode(object):
    ...
    def _buildNetwork(self):
        ...
        '''
        self.lr = tf.train.exponential_decay(0.001, self.global_step, decay_step, 0.996, staircase=True)
        self.train_step = tf.train.AdamOptimizer(self.lr).minimize(self.cross_entropy, global_step=self.global_step)
    ...
    def updateReader(self):
        zero = tf.constant(0)
        assign_zero = tf.assign(self.global_step, zero)
        print 'global step', self.sess.run(assign_zero)
