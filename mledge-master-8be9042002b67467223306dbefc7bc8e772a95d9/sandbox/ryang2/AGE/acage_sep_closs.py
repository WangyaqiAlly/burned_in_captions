# CUDA_VISIBLE_DEVICES='1' python acage_balancedloss.py
import argparse
import struct
import time
import sys
...
def enet(args, x, reuse=None):
    ...
    with tf.variable_scope('enet',reuse=reuse):
        ...
        ac_output = tf.contrib.layers.fully_connected(e, args.cat_dim, activation_fn=None)
        ...
        if len(one_class_all_ids) != 0:
            ...
            if imgs is None:
                ...
                if fake_label is not None:
                    show_label = np.array([fake_label[one_class_num_ids]])
            else:
...
ac_lamda = 1.0
# closs = 10 * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.concat([l, z_cat], axis=0), logits=tf.concat([e_l, e_g_z_l], axis=0)))
closs_real = 10 * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.squeeze(l), logits=tf.squeeze(e_l)))
closs_fake = 10 * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.squeeze(z_cat), logits=tf.squeeze(e_g_z_l)))
...
# egrads = eopt.compute_gradients(eloss + closs_real - closs_fake, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,'enet'))
egrads = eopt.compute_gradients(eloss + closs_real, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,'enet'))
...
ggrads = gopt.compute_gradients(gloss + closs_fake, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,'gnet'))
    ...
    for i in range(args.epochs):
        ...
        for j in range(0,int(d.shape[0])/int(args.batch)*args.batch,args.batch):
            ...
            for _ in range(args.ne_updates):
                ...
                _, el, en, kl_real_, kl_fake_e_, match_x_, ex_mean_, ex_var_, egz_mean_, egz_var_, egzl_, cl_real_ = sess.run([
                    etrain,eloss,enorm,
                    kl_real, kl_fake_e, match_x, 
                    ex_mean, ex_var, egz_mean, egz_var, 
                    e_g_z_l, closs_real], 
                    feed_dict={x:input_x, 
                        z:input_z,
                        l:input_l,
            ...
            for _ in range(args.ng_updates):
                ...
                _, gl, gn, kl_fake_g_, match_z_, cl_fake_ = sess.run([gtrain,gloss,gnorm, kl_fake_g, match_z, closs_fake], 
                    feed_dict={x:input_x, 
                            z:input_z,
                            l:input_l,
                            z_cat: input_zcat})
            ...
            print 'epoch %d image number %.5f ecost %.5f gcost %.5f cl_real %.5f cl_fake %.5f'% (i, j, el/t, gl/t, cl_real_/t, cl_fake_/t)
            ...
            print 'egzl_[0]:', np.squeeze(egzl_)[0]
