# CUDA_VISIBLE_DEVICES='1' python wage_resnet_kloss.py
...
PLOT_FREQUENCY = 200
load_dir = None
Save_dir = './wage_resnet_kloss/rate_'+str(Data_rate)+'/'
...
def age_loss(samples, direction = 'paper'):
...
with tf.Session() as session:
    ...
    for i, device in enumerate(DEVICES_A):
        with tf.device(device):
            ...
            # 'kl_loss'
            age_real_cost, mean_real, var_real = age_loss(enc_real)
            age_fake_cost, mean_fake, var_fake = age_loss(enc_fake)
            disc_costs.extend([5 * age_real_cost])
            gen_costs.extend([5 * age_fake_cost])
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(N_CRITIC):
            ...
            if CONDITIONAL and ACGAN:
                _disc_cost, _disc_wgan, _disc_acgan, _disc_acgan_acc, _disc_acgan_fake_acc, _, _enc_fake, _match_real, _match_fake, _age_real, _mean_real, _var_real, _age_fake, _mean_fake, _var_fake = session.run([
                    disc_cost, disc_wgan, disc_acgan, 
                    disc_acgan_acc, disc_acgan_fake_acc, disc_train_op,
                    enc_fake, match_real_cost, match_fake_cost,
                    age_real_cost, mean_real, var_real, age_fake_cost, mean_fake, var_fake], 
        ...
        if iteration % 10 == 9:
            ...
            wcost = _disc_wgan - _match_real - _match_fake - 5*_age_real
            gcost = _gen_cost - _match_real - _match_fake - 5*_age_fake

            print '\n----------Iteration %d ---------' % iteration
            print 'ecost', _disc_cost, 'wcost', wcost, 'lcost', _disc_acgan
            print 'kl_real %.5f mean %.5f var %.5f' % (5*_age_real, _mean_real, _var_real)
            print 'kl_fake %.5f mean %.5f var %.5f' % (5*_age_fake, _mean_fake, _var_fake)
