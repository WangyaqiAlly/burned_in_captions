# CUDA_VISIBLE_DEVICES='0' wage_21match.py
...
PLOT_FREQUENCY = 500
load_dir = None
Save_dir = './wage_21match/rate_'+str(Data_rate)+'/'
...
with tf.Session() as session:
    ...
    for i, device in enumerate(DEVICES_A):
        with tf.device(device):
            ...
            gen_costs.extend([match_fake_cost])
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration % 10 == 9:
            ...
            wcost = _disc_wgan - _match_real - _match_fake
            gcost = _gen_cost - _match_fake
        ...
        if iteration % PLOT_FREQUENCY == PLOT_FREQUENCY - 1:
            ...
            if CONDITIONAL and ACGAN:
                lib.plot.plot(Img_dir+'wgan_', wcost)
        ...
        if iteration % PLOT_FREQUENCY == PLOT_FREQUENCY - 1:
            ...
            lib.plot.plot(Img_dir+'dev_cost', np.mean(dev_disc_costs))
            lib.plot.plot(Img_dir+'acgan_acc', np.mean(disc_acgan_accs_))
