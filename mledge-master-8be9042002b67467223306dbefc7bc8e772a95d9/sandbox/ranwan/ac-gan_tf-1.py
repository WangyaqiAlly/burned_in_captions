...
def save_img(img,fig_name):
    ...
    for i in xrange(row):
        for j in xrange(col):
            #print i,j            
            ax[i][j].imshow(np.squeeze(img[i * row + j]), 'gray')
            ax[i][j].set_axis_off()
...
with tf.Session() as sess:
    ...
    for i in range(args.epochs):
        ...
        if i%args.save_img_freq ==0:
            for d in xrange(10):
                temp_z_cat = creat_z_cat_nonrand(args,num_sample=args.batch,digit=d)
                ...
                x0,l0,cat_enet = sess.run([gx,gl,cat_fake],feed_dict={x:data[j:j+args.batch],
                                                                        ...
                                                                        z_con : np.random.normal(size=(args.batch, args.con_dim)),
                                                                        z_rand : np.random.normal(size=(args.batch, args.m))})
                save_img(x0,'epoch%d_digit_%d.png' %(i,d))
                plt.close()
