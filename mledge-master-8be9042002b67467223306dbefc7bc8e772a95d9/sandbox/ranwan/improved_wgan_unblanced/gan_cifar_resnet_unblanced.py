...
Site_N = 1
if Site_N==0:
    label_prior = np.array([0.5,0.15,0.25,0.35,0.45,0.55,0.65,0.75,0.85,0.95])
    label_prior = label_prior/label_prior.sum()
elif Site_N==1:
    label_prior = 1.0-np.array([0.5,0.15,0.25,0.35,0.45,0.55,0.65,0.75,0.85,0.95])
    label_prior = label_prior/label_prior.sum()
...
load_dir = None
Save_dir = './Record/site_'+str(Site_N)+'/'
...
def OptimizedResBlockDisc1(inputs):
    ...
    shortcut = conv_shortcut('site_'+str(Site_N)+'/'+'Discriminator.1.Shortcut', input_dim=3, output_dim=DIM_D, filter_size=1, he_init=False, biases=True, inputs=inputs)

    output = inputs
    output = conv_1('site_'+str(Site_N)+'/'+'Discriminator.1.Conv1', filter_size=3, inputs=output)    
    output = nonlinearity(output)            
    output = conv_2('site_'+str(Site_N)+'/'+'Discriminator.1.Conv2', filter_size=3, inputs=output)
    return shortcut + output
...
def Discriminator(inputs, labels):
    ...
    output = ResidualBlock('site_'+str(Site_N)+'/'+'Discriminator.2', DIM_D, DIM_D, 3, output, resample='down', labels=labels)
    output = ResidualBlock('site_'+str(Site_N)+'/'+'Discriminator.3', DIM_D, DIM_D, 3, output, resample=None, labels=labels)
    output = ResidualBlock('site_'+str(Site_N)+'/'+'Discriminator.4', DIM_D, DIM_D, 3, output, resample=None, labels=labels)
    output = nonlinearity(output)
    output = tf.reduce_mean(output, axis=[2,3])
    output_wgan = lib.ops.linear.Linear('site_'+str(Site_N)+'/'+'Discriminator.Output', DIM_D, 1, output)
    output_wgan = tf.reshape(output_wgan, [-1])
    if CONDITIONAL and ACGAN:
        output_acgan = lib.ops.linear.Linear('site_'+str(Site_N)+'/'+'Discriminator.ACGANOutput', DIM_D, 10, output)
...
with tf.Session() as session:
    ...
    train_gen, dev_gen = lib.cifar10.load(BATCH_SIZE, DATA_DIR,Site_N,testing=True,test_dir = TEST_DATA_DIR)
    def inf_train_gen():
        while True:
    ...
    for iteration in xrange(ITERS):
        ...
        if iteration > 0:
            fake_labels_=np.random.choice(np.asarray(xrange(10)),BATCH_SIZE,p=label_prior)
            _ = session.run([gen_train_op], feed_dict={_iteration:iteration,fake_labels:fake_labels_})
        ...
        if iteration % 100 == 99:
            ...
            lib.plot.plot('acgan_acc', np.mean(disc_acgan_accs_))
