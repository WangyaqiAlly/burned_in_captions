...
class CentralNode(object):
    ...
    def classifier(self):
        with tf.variable_scope("classifier"):
            e = tf.layers.conv2d(inputs=x, filters=16, kernel_size=5, strides=2,activation=None, padding='same') ; print e
            e = leaky_relu(e)
            e = tf.nn.dropout(e,keep_rate)
            e = tf.layers.conv2d(inputs=e, filters=32, kernel_size=4, strides=1,activation=None, padding='same') ; print e
            ...
            e = tf.layers.conv2d(inputs=e, filters=64, kernel_size=4, strides=2,activation=None, padding='same') ; print e
            e = tf.contrib.layers.batch_norm(e,center=True,scale=True,is_training=phase,scope='bn2')
            e = leaky_relu(e)
            e = tf.nn.dropout(e,keep_rate)
            
            e = tf.layers.conv2d(inputs=e, filters=128, kernel_size=4, strides=1,activation=None, padding='same') ; print e
            ...
            e = tf.layers.conv2d(inputs=e, filters=256, kernel_size=4, strides=2,activation=None, padding='same'); print e
            e = tf.contrib.layers.batch_norm(e,center=True,scale=True,is_training=phase,scope='bn4')
            e = leaky_relu(e)
            e = tf.nn.dropout(e,keep_rate)
            
            e = tf.layers.conv2d(inputs=e, filters=512, kernel_size=4, strides=1,activation=None, padding='same') ; print e
            e = tf.contrib.layers.flatten(e) ; print e
            e = tf.layers.dense(inputs=e,units=1024, activation=tf.nn.elu);
            e = tf.layers.dense(inputs=e,units=1024, activation=tf.nn.elu);
    ...
    def train(self, total_epoch=100):
        ...
        elif self.use_fake_images:
            self._reader.update_images()
            self.train_dataset = self._reader.train
        ...
        for i in range(total_epoch):
            ...
            for j in range(0, data_test.shape[0], self.batch_size):
                p = self.sess.run(self.epred, feed_dict={self.x: data_test[j:j + self.batch_size]})
    ...
    # ===================Rarely use==================
    #
    def save_model(self, record_path, center_id='', time=''):
        saver = tf.train.Saver()
        if center_id != '':
            save_path = saver.save(self.sess, os.path.join(record_path, 'center'+center_id, time, 'model.ckpt'))
        else:
            save_path = saver.save(self.sess, os.path.join(record_path, 'center', 'model.ckpt'))
        self.logger.info("Model saved in file: %s" % save_path)
    def restore_model(self, record_path, center_id='', time=''):
        saver = tf.train.Saver()
        save_path = os.path.join(record_path, 'center'+center_id, time, 'model.ckpt')
