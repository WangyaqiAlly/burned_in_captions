...
parser.add_argument('--save_img_freq',help='frequency to same sampled images',default=10,type=float)
parser.add_argument('--save_model_freq',help='frequency to same sampled model',default=50,type=float)
parser.add_argument('--img_rep',help='replication of digits shown',default=100,type=int)
parser.add_argument('--name',help='name of this run, will create a folder to save images of the same name',default=strftime("%Y-%m-%d_%H-%M-%S", gmtime()),type=str)
parser.add_argument('--dataset',help='run on which dataset',default='cifar10',type=str)
parser.add_argument('--load_dir',help='which directory to load pretrained model', default=None,type=str)
parser.add_argument('--which_gan',help='select which gan to use: energy=energy based, vanilla=dcgan', default='energy',type=str)
parser.add_argument('--margin',help='margin in energy loss',default=1.0,type=float)

args = parser.parse_args()
args.save_img_freq = np.asarray(args.save_img_freq/args.data_fraction).astype('int32')
print args

from model_cifar10 import gnet
if args.which_gan== 'energy':
    from model_cifar10 import enet_energy, pullaway_loss
elif args.which_gan== 'vanilla':
    from model_cifar10 import enet

save_dir = "./asset/"+'_labelrate='+str(args.label_rate)+'_datafraction='+str(args.data_fraction)+'_'+args.dataset+'_'+args.name+"/"
...
if args.dataset=='cifar10':
    file_name = 'cifar_data/'+ 'cifar10.hdf'
    with h5py.File(file_name,'r') as fo:
        data = np.asarray(fo['images']).astype('float32')
        data = data/255.-0.5
        label= np.asarray(fo['labels']).astype('int32')
    ...
    #    label = np.dot(label_sel,label2)
        for isel in range(label.shape[0]):
            for jsel in range(label.shape[1]):
                label[isel][jsel] = label[isel][jsel]*label_sel[isel]
        data_labeled = data[label_sel==1]
        label_labeled = label[label_sel==1]    
...
x_sup = tf.placeholder('float32', [None,32,32,3],name='x_sup') ; print x_sup
l_sup = tf.placeholder('float32', [None,args.cat_dim],name='l_sup') ; print l_sup
...
if args.which_gan=='vanilla':
    _,cat_real_sup,_ = enet(args,x_sup)
    ex,cat_real,_ = enet(args,x,reuse=True) # e(x)
elif args.which_gan=='energy':
    _,cat_real_sup,_,_= enet_energy(args,x_sup)
    ex,cat_real,_,_ = enet_energy(args,x,reuse=True) # e(x)
gx = gnet(args,z) # g(z)

if args.which_gan=='vanilla':
    egz,cat_fake,con_fake = enet(args,gx,reuse=True) # e(g(z))
elif args.which_gan=='energy':
    egz,cat_fake,con_fake,embeddings_fake = enet_energy(args,gx,reuse=True) # e(g(z))
...
if args.which_gan=='vanilla':
    ...
    zero = tf.zeros_like(args.margin-egz)
    eloss = ex + tf.maximum(zero, args.margin-egz)

# generate loss
if args.which_gan=='vanilla':
    gloss = -tf.reduce_mean(tf.log(egz))
elif args.which_gan=='energy':
    pt_loss = pullaway_loss(args,embeddings_fake)
    gloss = egz + 0.1*pt_loss
...
# closs = 0.5* tf.nn.softmax_cross_entropy_with_logits(labels=z_cat,logits=cat_fake) \
#        +0.5* tf.nn.softmax_cross_entropy_with_logits(labels=l_sup,logits=cat_real_sup)
...
if args.which_gan=='energy':
    egrads = eopt.compute_gradients(eloss + closs + conloss ,var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,'enet_energy'))
...
def save_img(img,fig_name,save_dir):
    ...
    for i in xrange(row):
        for j in xrange(col):
            #print i,j            
            ax[i][j].imshow((np.clip(np.squeeze(img[i * row + j])+0.5,0.,1.)*255.).astype('uint8'))
            ax[i][j].set_axis_off()
            if i*row+j >= len(img)-1:
                break
...
with tf.Session() as sess:
    ...
    for i in range(args.epochs):
        ...
        for j in range(0,data.shape[0],args.batch):
            ...
            labeled_ind = np.where(np.sum(label,axis=1))[0]
            suffled_ind = np.random.randint(len(labeled_ind), size=args.batch)

            _,el_,en_ = sess.run([etrain,eloss,enorm],feed_dict={x:data[j:j+args.batch],
                                                                 l:label[j:j+args.batch],
                                                                 x_sup:data[labeled_ind[suffled_ind]],
                                                                 l_sup:label[labeled_ind[suffled_ind]],
        ...
        if i%args.save_img_freq ==0:
            for d in xrange(10):
                temp_z_cat = creat_z_cat_nonrand(args,num_sample=args.img_rep,digit=d)

                x0,cat_enet = sess.run([gx,cat_fake],feed_dict={z_cat: temp_z_cat,
                                                                      z_con : np.random.normal(size=(args.img_rep, args.con_dim)),
                                                                      z_rand : np.random.normal(size=(args.img_rep, args.m))})

                save_img(x0,'epoch%d_%s.jpg' %(i,label_name[d]),sample_dir)
                #plt.close()
        if i%args.save_model_freq ==0:
            global_steps = sess.run(global_step)
            print '-'*30
            print 'Saving models...'
            saver.save(sess,model_dir+'model.ckpt',global_step=global_steps)
            print 'Saved models'
            print '-'*30
       ...
       # print loo
        ...
        #     cv2.putText(img, "{:d}".format(np.argmax(cat_enet[k])),(k*100,95),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,255,0),2)
        # cv2.imshow('img',img)
        # cv2.waitKey(10)
