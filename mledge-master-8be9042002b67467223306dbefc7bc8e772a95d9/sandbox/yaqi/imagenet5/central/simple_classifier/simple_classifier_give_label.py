...
if tag.endswith('.py'):
    tag = tag[:-3]
    tag = tag + '_' + str(TRAIN_SET_PCT*100)
...
#### Read training data
#train_data = np.load('./data/train/images.npy')
#train_labels = np.load('./data/train/labels.npy')

#train_num = int(train_data.shape[0] * TRAIN_SET_PCT)
#idx = random.sample(range(train_data.shape[0]), train_num)
#train_data = train_data[idx]
#train_labels = train_labels[idx]
#print "!!!!!!!!!train_deat_shape:", train_data.shape[0]
...
test_ckpt_path='./simple_classifier_1_100/model99'
data_to_label = np.load('../ALI/uncon_gen/uncon_gen.npy')

label_save_path='../ALI/uncon_gen/'
...
with tf.Session() as sess:
 ...
 #            f_testing.write('Epoch: ' + str(epoch) + ' test_acc: ' +  str(acc) + '\n')
    ...
    counter = 0
    print 'data_to_albel.shape:', data_to_label.shape
    labs=None
    for batch in range(0, data_to_label.shape[0], batch_size):
        counter += 1
        input_x_ori = data_to_label[batch:batch+batch_size]
        input_x = np.clip(input_x_ori ,0.,1.) - 0.5
        #input_y = vali_labels[batch:batch+batch_size]
        lab = sess.run(prediction, feed_dict = {x: input_x, keep_prob:1.0})
        print lab[0]
    ...
    #np.save(label_save_path + 'labels', labs)
