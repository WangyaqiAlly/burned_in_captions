...
from resnet_5_class import *
from datetime import datetime
import time
from cifar10_input import *
import os

from load_imgnet5 import load_imgnet5_train, load_imgnet5_val

#import pandas as pd
USE_AUGMENTATION = False
TRAIN_SET_PCT=80
TOTAL_TRAIN_NUM=240000

Scope_Prefix = 'golden_imgnet5_'+str(TRAIN_SET_PCT)
Total_iteration = 100000
Total_batchn = int(1875 *TRAIN_SET_PCT/100)  #total train num/batch
print "training set percent:",TRAIN_SET_PCT,"%",\
    "\n Total_batchn :",Total_batchn
Record_path='./record/'+ Scope_Prefix+'/'
if not os.path.exists(Record_path):
    os.makedirs(Record_path)

txt_path=  Record_path+'txt/'
if not os.path.exists(txt_path):
    os.makedirs(txt_path)

model_save_path = Record_path+'model/'
if not os.path.exists(model_save_path):
    os.makedirs(model_save_path)

Record_file = txt_path+ Scope_Prefix +'record.txt'
acc_file_name = txt_path+Scope_Prefix + 'record_each_class.txt'
acc_file_name_train = txt_path+Scope_Prefix +'record_each_class_train.txt'
mtx_file_name = txt_path+Scope_Prefix + 'confusion_mtx.txt'

datadir='./data/imagenet-plus-5-class/pfiles/'
import cPickle
...
class Train_site0(object):
    ...
    def build_train_validation_graph(self):
        ...
        logits = inference(self.image_placeholder, FLAGS.num_residual_blocks, reusef=False,id=self.id, keep_rate = 1)
        vali_logits = inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reusef=True,id=self.id, keep_rate = 1)
    ...
    def train(self):
        ...
        d, labels = load_imgnet5_train(datadir=datadir, shuffle=True,percent=TRAIN_SET_PCT)

        # print 'data_shape',all_data.shape
        vali_data, vali_labels = load_imgnet5_val(datadir=datadir,shuffle=True)
        vali_data = whitening_image(vali_data)
        print 'validation:  vali_data.shape: ',vali_data.shape, ' vali_labels.shape: ', vali_labels.shape, 'vali_labels.type: ', type(vali_labels)
        #vali_data, vali_labels = read_validation_data_ali()
        ...
        for step in xrange(Total_iteration):
            ...
            train_batch_data = d[load_n*128:load_n*128+128]
            train_batch_labels = labels[load_n*128:load_n*128+128]
            train_batch_data = self.generate_augment_train_batch(train_batch_data,augment = USE_AUGMENTATION)
            ...
            if step % FLAGS.report_freq == 0:
                ...
                if FLAGS.is_full_validation is True:
                    ...
                    facc.write("%09d %2.8f %2.8f %2.8f %2.8f %2.8f \n" % (step, acc_each_c_mean[0],acc_each_c_mean[1],acc_each_c_mean[2],acc_each_c_mean[3],\
                    acc_each_c_mean[4]))
                    facc.close()

                    if step % (FLAGS.report_freq*10) == 0:

                        vali_predict_fraction = np.zeros((5,5))
                        for mtxii in xrange(5):
                            tempsum = 0
                            for mtxjj in xrange(5):
                        ...
                        for mtxi in xrange(5):
                            fmtx.write("%.4f %.4f %.4f %.4f %.4f \n" % (vali_predict_fraction[mtxi,0],vali_predict_fraction[mtxi,1],\
                                        vali_predict_fraction[mtxi,2],vali_predict_fraction[mtxi,3],vali_predict_fraction[mtxi,4]) )
                        fmtx.close()
            ...
            if step % FLAGS.report_freq == 0:
                eclass_wrong_train = np.zeros(5)
                eclass_right_train = np.zeros(5)
                ...
                acc_each_c_mean_train = np.zeros(5)
                for ci in xrange(5):
    ...
    def generate_augment_train_batch(self, batch_data, augment = False):
        '''
        ...
        '''
        # batch_data = np.load(data_dir+'img'+('%06d' %(batch_num))+'.npy')
        ...
        # batch_label_one_hot = np.load(data_dir+'labels'+('%06d' %(batch_num))+'.npy')
        # batch_label = np.argmax(batch_label_one_hot,axis = 1)
    ...
    def full_validation(self, loss, top1_error, session, vali_data, vali_labels, batch_data,
        ...
        :return: float, float
        '''
        num_batches = 60000 // FLAGS.validation_batch_size
        order = np.random.choice(60000, num_batches * FLAGS.validation_batch_size)
        ...
        eclass_wrong = np.zeros(5)
        eclass_right = np.zeros(5)
        predict_mtx = np.zeros((5,5))

        for step in range(num_batches):

            offset = step * FLAGS.validation_batch_size
            #print vali_data_subset[offset:offset+FLAGS.validation_batch_size, ...].shape
            #print vali_labels_subset[offset:offset + FLAGS.validation_batch_size]
