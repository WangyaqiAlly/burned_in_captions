...
tf.app.flags.DEFINE_string('unlabeled_type', 'CTGAN_SEMI','which site')
tf.app.flags.DEFINE_string('sites', ['0','2','4','5'],'which site')
tf.app.flags.DEFINE_integer('site_num', 4,'which site')
...
tf.app.flags.DEFINE_string('log_dir', "./Record/CTGAN_SEMI/site_0_2_4_5/", "log_dir")
tf.app.flags.DEFINE_bool('is_load_pretrained_model',False,"")
tf.app.flags.DEFINE_string('pretrained_dir', "./Record/CTGAN_SEMI/site_0_2", "pretrained_dir")
...
def main(_):
    with tf.Graph().as_default() as g:
        NUM_ITER_PER_EPOCH = int(FLAGS.train_size_per_pct * 10*FLAGS.site_num/ FLAGS.ul_batch_size)
            ...
            for ep in range(FLAGS.num_epochs):
                ...
                for i in range(NUM_ITER_PER_EPOCH):
                    ...
                    if (i + 1) % FLAGS.iter_report_freq == 0:
                        logger.info("Epoch:{} Iter: {}, CE_loss_train:{} ".format(ep, i, sum_loss/i))
                    ...
                    if (i + 1) % FLAGS.eval_freq == 0 or (i + 1) == NUM_ITER_PER_EPOCH:
                        ...
                        logger.info("-----site:{} type:{}, max_test_acc:{},iter:{}------------".format(
                                 FLAGS.sites, FLAGS.unlabeled_type,max_test_acc,max_test_acc_iter))
                end = time.time()
