...
N_CRITIC = 3 # Critic steps per generator steps
...
def Normalize(name, inputs,labels=None):
    ...
    if ('Discriminator' in name) and NORMALIZATION_D:
        return lib.ops.layernorm.Layernorm(name,[1,2,3],inputs,labels=labels,n_labels=10) # changed from 10
    elif ('Generator' in name) and NORMALIZATION_G:
        if labels is not None:
            return lib.ops.cond_batchnorm.Batchnorm(name,[0,2,3],inputs,labels=labels,n_labels=10) # changed from 10
        else: 
...
def Generator(n_samples, labels, noise=None):
    ...
    output = ResidualBlock('Generator.4', DIM_G, DIM_G, 3, output, resample=None, labels=labels)
    output = ResidualBlock('Generator.4', DIM_G, DIM_G, 3, output, resample=None, labels=labels)
    output = ResidualBlock('Generator.4', DIM_G, DIM_G, 3, output, resample=None, labels=labels)
...
def Discriminator(inputs, labels):
    ...
    if CONDITIONAL and ACGAN:
        output_acgan = lib.ops.linear.Linear('Discriminator.ACGANOutput', DIM_D, 5, output) ## changed from 10 
...
with tf.Session() as session:
    ...
    frame_i = [0]
    fixed_noise = tf.constant(np.random.normal(size=(50, 256)).astype('float32'))
    ...
    train_gen, dev_gen = lib.cifar10.load(BATCH_SIZE, DATA_DIR,Site_N)
    def inf_train_gen():
        while True:
    ...
    for iteration in xrange(ITERS):
        ...
        # print '_disc_wgan',_disc_wgan
        # print '_disc_acgan_acc',_disc_acgan_acc
        # print '_disc_acgan_fake_acc',_disc_acgan_fake_acc
