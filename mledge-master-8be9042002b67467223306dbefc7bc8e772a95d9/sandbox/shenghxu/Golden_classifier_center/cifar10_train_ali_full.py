...
class Train_site0(object):
    ...
    def generate_augment_train_batch(self, batch_num, augment = False):
        ...
        '''
        dir_prefix = 'data0/ali_cifar_conditional_100_256/'
        batch_data0 = np.load(dir_prefix+'img'+('%06d' % (batch_num*2) )+'.npy')
        batch_data1 = np.load(dir_prefix+'img'+('%06d' % (batch_num*2+1) )+'.npy')
        batch_data = np.concatenate((batch_data0,batch_data1),axis=0)
        #print 'data_max', np.max(batch_data)
        ...
        batch_label_one_hot0 = np.load(dir_prefix+'labels'+('%06d' % (batch_num*2) )+'.npy')
        batch_label_one_hot1 = np.load(dir_prefix+'labels'+('%06d' % (batch_num*2+1) )+'.npy')
        batch_label_one_hot = np.concatenate((batch_label_one_hot0,batch_label_one_hot1),axis=0)
        batch_label = np.argmax(batch_label_one_hot,axis = 1)
        #offset = np.random.choice(self.data_length - train_batch_size, 1)[0]
