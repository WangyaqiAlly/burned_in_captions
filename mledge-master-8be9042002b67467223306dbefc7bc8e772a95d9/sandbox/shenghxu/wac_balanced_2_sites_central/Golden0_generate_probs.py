...
# Scope_Prefix = 'ali_uncon'
Scope_Prefix = 'Site0_50_'
...
class Train_site0(object):
    ...
    def generate_label(self):
        ...
        saver.restore(sess, './models/model.ckpt-golden_site0')
        print 'Model restored from ', './models/model.ckpt-golden_site0'

        count = 0
        for b_n in xrange(60000):

             # for GAN0 generated images
           
            batch_data = np.load('./data0/img'+('%06d' % b_n)+'.npy')
            ...
            # Golden classifier trained on BGR images so we convert RGB to BGR           
            l = sess.run(predictions,feed_dict={self.test_image_placeholder: batch_data})
            
        
            np.save('./data0/gold_prob_site0'+('%06d' % b_n), l)

            # for GAN1 generated images

            batch_data = np.load('./data1/img'+('%06d' % b_n)+'.npy')
            ...
            np.save('./data1/gold_prob_site0'+('%06d' % b_n), l)
            count += 1
            if count % 100 == 0:
                print count
    ...
    def generate_augment_train_batch(self, batch_num, augment = False):
        ...
        '''
        batch_data = np.load('../data_uncon_100/'+'img'+('%06d' %(batch_num))+'.npy')
        ...
        batch_label_one_hot = np.load('../data_uncon_100/'+'labels'+('%06d' %(batch_num))+'.npy')
