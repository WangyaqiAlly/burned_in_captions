...
class Train_site0(object):
    ...
    def generate_label(self):
        ...
        saver.restore(sess, './models/model.ckpt-site1_50')
        print 'Model restored from ', './models/model.ckpt-site1_50'
        ...
        for b_n in xrange(60000):
            ...
            np.save('./data0/gold_prob_site1'+('%06d' % b_n), l)
            ...
            #l = np.argmax(l, axis = 1)
            #targets = targets.astype('uint8')
            np.save('./data1/gold_prob_site1'+('%06d' % b_n), l)
            count += 1
            if count % 100 == 0:
                print count
