...
train_dir = 'wac_Site0_1_gold_label_mix'
...
Scope_Prefix = 'wac_Site0_1_gold_label_mix'
...
class Train_site0(object):
    ...
    def train(self):
        ...
        for step in xrange(Total_iteration):
            ...
            train_batch_data, train_batch_labels, train_img_mixed, train_label_mixed = self.generate_augment_train_batch_mix(load_n,train_img_mixed, train_label_mixed,augment = USE_AUGMENTATION)
    ...
    def generate_augment_train_batch(self, batch_num, augment = False):
        ...
        batch_label_one_hot = np.load('./data0/'+'gold_labels'+('%06d' %(batch_num))+'.npy')
    ...
    def generate_augment_train_batch_mix(self, batch_num, batch_data_mixed, batch_label_mixed, augment = False):
        '''
        This function helps generate a batch of train data, and random crop, horizontally flip
        and whiten them at the same time
        :param train_data: 4D numpy array
        ...
        if batch_num % 2 ==0:
            ...
            batch_data_0 = np.load('./data0/'+'img'+('%06d' %(batch_num))+'.npy')
            batch_data_1 = np.load('./data1/'+'img'+('%06d' %(batch_num))+'.npy')
            batch_data_mixed = np.concatenate((batch_data_0,batch_data_1),axis=0)
            batch_label_0 = np.load('./data0/'+'gold_labels'+('%06d' %(batch_num))+'.npy')
            batch_label_1 = np.load('./data1/'+'gold_1_labels'+('%06d' %(batch_num))+'.npy')
            batch_label_mixed = np.concatenate((batch_label_0,batch_label_1),axis=0)
            rng_state = np.random.get_state()
            np.random.shuffle(batch_data_mixed)
            np.random.set_state(rng_state)
            np.random.shuffle(batch_label_mixed)

            batch_data = batch_data_mixed[0:128]
            batch_label = batch_label_mixed[0:128].astype(np.int32)
        else:
            batch_data = batch_data_mixed[128:256]
            batch_label = batch_label_mixed[128:256].astype(np.int32)
        ...
        return batch_data, batch_label, batch_data_mixed, batch_label_mixed
