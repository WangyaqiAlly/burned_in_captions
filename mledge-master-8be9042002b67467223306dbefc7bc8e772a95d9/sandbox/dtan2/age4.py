...
egrads_real = eopt.compute_gradients(ecost_real,var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,'enet'))
...
with tf.Session() as sess:
    ...
    for i in range(args.epochs):
        ...
        cv2.imshow('age4', cv2.resize(np.concatenate((x0[0:10]).astype('uint8'),axis=1),(1000,100)))
        cv2.waitKey(10)
