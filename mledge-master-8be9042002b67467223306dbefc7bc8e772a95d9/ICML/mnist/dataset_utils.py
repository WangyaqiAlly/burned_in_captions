...
tf.app.flags.DEFINE_bool('whitening', False, "")
...
def read(filename_queue,train):
    ...
    image = tf.image.per_image_standardization(image) if FLAGS.whitening else image
    print("whitening...")
    label = tf.one_hot(tf.cast(features['label'], tf.int32), 10)
    return image, label
...
def generate_batch_join(
    ...
    if shuffle:
        ret = tf.train.shuffle_batch_join(
            ...
            min_after_dequeue=dataset_size )#int(dataset_size/10))
    else:
        ret = tf.train.batch(
            example,
            batch_size=batch_size,
