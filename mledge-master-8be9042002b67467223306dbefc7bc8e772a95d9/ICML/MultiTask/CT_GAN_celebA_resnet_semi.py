...
parser.add_argument('--label', help='label for conditioning', default=None)
parser.add_argument('--site', help='site id', default=-1, type=int)
args = parser.parse_args()
print args

if args.label is None:
    print('--label must be supplied as one of the attributes of Celeb-A or the string Unconditional')
    sys.exit(1)
...
#tf.app.flags.DEFINE_string('save_dir', os.path.join('./Record', args.label), "log_dir")

Save_dir = os.path.join('./Record', args.label)
if args.site >= 0:
    Save_dir += '-site' + str(args.site)
#Save_dir = FLAGS.save_dir

explabel=args.label
if explabel == 'Unconditional':
    explabel = 'Smiling'

# Download CIFAR-10 (Python version) at
# https://www.cs.toronto.edu/~kriz/cifar.html and fill in the path to the
# extracted files here!
#DATA_DIR = '/home/yaqiwang/yaqi@sandbox/Data/cifar-10-batches-py'    #file path to be modified
#LABELED_DATA_DIR = '/home/yaqiwang/yaqi@sandbox/distributed_ml/Data/cifar_nozca/'    #file path to be modified
#if len(DATA_DIR) == 0:
#    raise Exception('Please specify path to data directory in gan_cifar.py!')
DATA_PATH = '/home2/dataset/Celeb-A'
...
OUTPUT_DIM = 3072*4 # Number of pixels in CIFAR10 (32*32*3)
LR = 2e-4 # Initial learning rate
DECAY = True # Whether to decay LR over learning
N_CRITIC = 5 # Critic steps per generator steps
#INCEPTION_FREQUENCY = 15000 # How frequently to calculate Inception score
MODEL_SAVE_FREQUENCY = 5000
CONDITIONAL = True # Whether to train a conditional or unconditional model
if args.label == 'Unconditional':
    CONDITIONAL = False
ACGAN = True # If CONDITIONAL, whether to use ACGAN or "vanilla" conditioning
ACGAN_SCALE = 0.1 # How to scale the critic's ACGAN loss relative to WGAN loss
...
def Discriminator(inputs, labels,kp1,kp2,kp3): # three more parameters of keep rate
    ...
    #output_c = ResidualBlock('Discriminator.5', DIM_D, DIM_D, 3, output, resample=None, labels=labels)
    #output_c = tf.nn.dropout(output_c, keep_prob=kp3)  # dropout after activator
    #output_c=nonlinearity(output_c)
    #output_c = tf.reduce_mean(output_c, axis=[2, 3])  # corresponding to D_
    ...
    if CONDITIONAL and ACGAN:
        output_acgan = lib.ops.linear.Linear('Discriminator.ACGANOutput', DIM_D, NUM_LABELS, output2)
        #output_acgan_new = lib.ops.linear.Linear('Discriminator.ACGANOutput', DIM_D, 10, output_c)
        return output_wgan, output2, output_acgan
        #return output_wgan, output2, output_acgan_new
    else:
...
with tf.Session() as session:
    ...
    fixed_labels = tf.constant(np.array([0,1]*50,dtype='int32'))
    fixed_noise_samples = Generator(100, fixed_labels, noise=fixed_noise)
    def generate_image(frame, true_dist):
        samples = session.run(fixed_noise_samples)
        samples = ((samples+1.)*(255./2)).astype('int32')
        lib.save_images.save_images(samples.reshape((100, 3, 64, 64)), Img_dir+'samples_{}.png'.format(frame))
    ...
    if args.site >= 0:
        unlabelled_dir = os.path.join(DATA_PATH, 'train-unlabelled-site-' + str(args.site))
        labelled_dir   = os.path.join(DATA_PATH, 'train-labelled-site-' + str(args.site))
    else:
        unlabelled_dir = os.path.join(DATA_PATH, 'train-unlabelled')
        labelled_dir = os.path.join(DATA_PAH, 'train-labelled')
    train_gen_lab, train_gen_unlab, dev_gen = celeb.read_data_semi(explabel, unlabelled_dir, labelled_dir,
                                                                   os.path.join(DATA_PATH, 'test'), os.path.join(DATA_PATH, 'list_attr_celeba.txt'), BATCH_SIZE)
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(N_CRITIC):
            ...
            if CONDITIONAL and ACGAN:
                ...
                if CONDITIONAL:
                    ...
                    _disc_cost, _ = session.run([disc_cost, disc_train_op], feed_dict={all_real_data_int_lab: _data, all_real_labels:_labels,all_real_data_int_unlab: _data_unlab, _iteration:iteration})
        ...
        if iteration % 100 == 99:
            ...
            for images,_labels in dev_gen():
                _dev_disc_cost,_disc_acgan_acc = session.run([disc_cost,disc_acgan_acc], feed_dict={all_real_data_int_lab: images, all_real_labels:_labels,all_real_data_int_unlab:images })
